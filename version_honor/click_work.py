...
def main(x):
    ...
    while True:
        # try:
        matches = pyautogui.locateAllOnScreen('./img/t6.png', region=region, confidence=0.8)
        click_1(matches, x)
        if not status_code:
            break
        #
        ...
        #     return
        # print(top_y_point)
        print(917 - top_y_point)
        pyautogui.scroll(int(917 - top_y_point))

        if int(917 - top_y_point) < 455:
            break
...
#     print(s)
#     main(s)
l1 = Label(top,text='自动点击，F9运行F10看坐标F11暂停')
l1.pack()
